...
std::vector<double> PrevX, PrevY, PrevZ, VelX, VelY, VelZ;  //a t0
...
void Print (NodeContainer VehicleUEs)  // non usata
{
        uint32_t ID;
        bool inside;
...
//--------------------------------------------------------------------------------------------
  ...
  uint32_t pscchLength = 8; // Length of the PSCCH
  std::string period="sf40";
  simTime = 30; // Tempo di simulazione
  double ueTxPower = 23.0; // UE transmission power in [dBm]
  ...
  uint32_t highwayLength = 5000; // 5km
...
// Change the random run  
  ...
  CommandLine cmd; // if not specified andefault value is assigned
...
//@DOC Not really needed: ns3 must set up the LTE infrastructure to run the simulation (at least the PDN-GW or PGW to assign IP addresses)
  ...
  /******************************************************************************************************************************************/
  /********************************************************* COMMENTED CODE BY FEDE *********************************************************/
  /******************************************************************************************************************************************/
  //@DOC: Now the UEs (actually, V-UEs are created)
  NodeContainer ueResponders; 		// Creation of the NodeContainer (it is a class) called ueResponders;
									// in this container are added responder nodes with a for loop.
  
  
  
  /************************************************************************************************************/
  /******************** This for loop is used to create and add nodes to the NodeContainer. *******************/
  /************************************************************************************************************/
  for (uint32_t t=0; t<ueCount; ++t)
  {
    Ptr<Node> lteNode = CreateObject<Node> (); 						// Creation of a new object called Node, it is empty.
																	// Node is the type of the object that is created.
																	// lteNode is a pointer to the new object that has been
																	// created by the function (i.e. method) CreateObject, the object is called Node.
    Ptr<LTENodeState> nodeState = CreateObject<LTENodeState> (); 	// LTENodeState is a new class developed from scratch. Defined before the main().
																	// nodeState is a pointer to the new object that has been created by the function
																	// (i.e. method) CreateObject, the object is called LTENodeState.
    nodeState -> SetNode(lteNode); 									// Using the arrow operator it is possible to take the content of the memory cell
																	// at the address nodeState. Take the object "LTENodeState" saved in nodeState and
																	// select the function "SetNode()" of it. The function has as argument lteNode and
																	// it is used to set a new node.
    lteNode -> AggregateObject(nodeState); 							// Aggregate two objects together. Now it's possible to call "GetObject()" on one
																	// to get the other and vice-versa.
    ueResponders.Add(lteNode); 										// The node is added to the container
  }
  /************************************************************************************************************/
  
 
 
  MobilityHelper mobilityUE; 															// It is a helper class, called mobilityUE used to assign positions and mobility models to nodes
  Ptr<UniformRandomVariable> laneNumber = CreateObject<UniformRandomVariable>(); 		// Initialization of a new object called laneNumber of type UniformRandomVariable, it is empty.
																						// "laneNumber" is a pointer to the new object
  Ptr<UniformRandomVariable> Xposition = CreateObject<UniformRandomVariable>(); 		// Initialization of a new object called XPosition of type UniformRandomVariable, it is empty.
																						// "Xposition" is a pointer to the new object.
  double laneWidth = 4.0;																// The lane width is fixed to 4.0 m, it is a double beacuse we want also decimals.
  Ptr<ListPositionAllocator> positionAlloc = CreateObject <ListPositionAllocator>(); 	// Initialization of a position allocator, it will contain the position of the vehicles, now it is empty.
																						// "positionAlloc" is a pointer to the new object.
  //int pospos = 1;
  //GeoCellSize = 8;
  ...
  /************************************************************************************************************/
  /********        This for loop it is used to define the x and the y position of each vehicle.       *********/
  /********          For the x position a value is chosen between 0 and 5km (highway length).         *********/
  /********   For the y position the lane width is multiplied for an integer chosen between 1 and 6.  *********/
  /************************************************************************************************************/
  for (NodeContainer::Iterator L = ueResponders.Begin(); L != ueResponders.End(); ++L) 
  {  
    double yPos = laneNumber->GetInteger(1,6)*laneWidth; 								// It saves the y position in a double called "yPos".
																						// It uses the function "GetInteger(1,6)" that is in the object pointed by "laneNumber".
//  double yPos = laneNumber->GetInteger(1,1)*laneWidth; // 1 lane simulation
    double xPos = Xposition->GetValue(0,highwayLength); 								// It saves the x position in a double called "xPos".
																						// It uses the function "GetValue" that is in the object pointed by "Xposition".
//  double xPos = Xposition->GetValue(0,ReuseDistance);
//  double xPos = pospos;
//  pospos += GeoCellSize;
//  xPos += 2000;
    positionAlloc->Add(Vector(xPos, yPos, 0)); 											// The vector with the positions is added to the position allocator
  }
  /************************************************************************************************************/
  
  
  
  mobilityUE.SetPositionAllocator(positionAlloc); 										// The position allocator is set on the mobilityUE helper
  mobilityUE.SetMobilityModel("ns3::ConstantVelocityMobilityModel");					// The mobilty model is set on the mobilityUE helper
//mobilityUE->SetVelocity({20,0,0});
  mobilityUE.Install(ueResponders); 													// Installation on the responding UEs



  /************************************************************************************************************/
  /*********** This for is used to scan nodes in the NodeContainer and set their constant velocity. ***********/
  /************************************************************************************************************/
  for (NodeContainer::Iterator L = ueResponders.Begin(); L != ueResponders.End(); ++L)
  {  
    Ptr<Node> node = *L;																			// Used to scan all nodes in NodeContainer.
    Ptr<MobilityModel> mob = node->GetObject<MobilityModel> (); 									// The function GetObject is used to get the pointer to the function MobilityModel,
																									// The pointer to the MobilityModel is saved in "mob" (i.e. type = MobilityModel).
    Ptr<ConstantVelocityMobilityModel> VelMob = node->GetObject<ConstantVelocityMobilityModel>(); 	// Acctual position: MobilityModel. "GetObject" is used to extrac the pointer of the
																									// object "ConstantVelocityMobilityModel". The pointer is saved in "VelMob"
																									// (i.e. type = ConstantVelocityMobilityModel)
    if (mob->GetPosition().y > 13) 																	// Using the pointer "mob" it is possible to select the function GetPosition() of the
																									// object "MobilityModel". The syntax ".y" is added to get the y position.
																									// "GetPosition()" returns all the vector xyz, but if ".y" is explicited only the "y" element is taken.
																									// If the node is located in the upper 3 lane, then the condition is true.
      VelMob->SetVelocity(Vector(19.44, 0, 0));														// Using the pointer "VelMob" it is possible to select the function "SetVelocity(Vector(vx,vy,vz))" of
																									// the object "ConstantVelocityMobilityModel" to set the positive velocity in the upper three lanes.
//    VelMob->SetVelocity(Vector(0, 0, 0));     
    else
      VelMob->SetVelocity(Vector(-19.44, 0, 0)); 													// Using the pointer "VelMob" it is possible to select the function "SetVelocity(Vector(vx,vy,vz))" of
																									// the object "ConstantVelocityMobilityModel" to set the negative velocity in the lower three lanes.
//    VelMob->SetVelocity(Vector(0, 0, 0));  
  }
  /************************************************************************************************************/
...
*/
  ...
  /************************************************************************************************************/
  /********** This for is used to scan nodes in the NodeContainer and to manage each node velocity. ***********/
  /************************************************************************************************************/
  for (NodeContainer::Iterator L = ueResponders.Begin(); L != ueResponders.End(); ++L)
  {  
    int ID;																				// int variable declaration.
    Ptr<Node> node = *L;																// Used to scan all nodes in NodeContainer.
    ID = node->GetId ();																// Used to select the function "GetId()" located in "node".
																						// "GetId()" returns an integer number (i.e. ID address of the node).
//  std::cin.get();
    Ptr<MobilityModel> mob = node->GetObject<MobilityModel> ();							// The function GetObject is used to get the pointer to the function MobilityModel,
																						// The pointer to the MobilityModel is saved in "mob" (i.e. type = MobilityModel).
    if (! mob) continue; 																// It is used to jump at the end of the current cycle if a node has no mobility model;
																						// it jumps to the next node and the following operations are not done.
    Vector pos = mob->GetPosition ();													// A Vector called "pos" is defined and the position of the current node is saved in it;
																						// using the pointer mob and the arrow operator it is possible to use the function
																						// "GetPosition()" which returns the x, y, and z positions of the node.
    PrevX.push_back(pos.x);																// PrevX is a c++ vector which is initially empty. Using the command "push_back(pos.x)"
																						// the vector is filled by the x position of the current node. At the end of the for loop
																						// this vector is "number of nodes - 1" long (the first node is 1 insted the first
																						// array position is 0).
    PrevY.push_back(pos.y);																// PrevY is a c++ vector which is initially empty. Using the command "push_back(pos.y)"
																						// the vector is filled by the y position of the current node. At the end of the for loop
																						// this vector is "number of nodes - 1" long (the first node is 1 insted the first
																						// array position is 0).
    PrevZ.push_back(pos.z);																// PrevZ is a c++ vector which is initially empty. Using the command "push_back(pos.z)"
																						// the vector is filled by the z position of the current node. At the end of the for loop
																						// this vector is "number of nodes - 1" long (the first node is 1 insted the first
																						// array position is 0).
    VelX.push_back(0);																	// VelX is a c++ vector which is initially empty. Using the command "push_back(0)"
																						// the vector is filled with 0 (i.e the velocity of this node is zero). Note that
																						// this is a separate vector, so the velocity stored in the mobility model is not changed.
    VelY.push_back(0);																	// VelY is a c++ vector which is initially empty. Using the command "push_back(0)"
																						// the vector is filled with 0 (i.e the velocity of this node is zero). Note that
																						// this is a separate vector, so the velocity stored in the mobility model is not changed.
    VelZ.push_back(0);																	// VelZ is a c++ vector which is initially empty. Using the command "push_back(0)"
																						// the vector is filled with 0 (i.e the velocity of this node is zero). Note that
																						// this is a separate vector, so the velocity stored in the mobility model is not changed.
//  PrevX[ID-1] = pos.x;
//  PrevY[ID-1] = pos.y;
//  PrevZ[ID-1] = pos.z;
	/************************************************************************************************************/
	/******************  Still in the previous for; this part of the code is used to fill the  ******************/
	/****************** "VehicleTrafficType" vector node by node with the desired traffic type. *****************/
	/******************  Note that: 0x00 --> periodic traffic, and 0x01 --> aperiodic traffic  ******************/
	/************************************************************************************************************/
    if ((AperiodicTraffic) || (ETSITraffic))						// Is the traffic aperiodic? "AperiodicTraffic" is a boolen variable acquired from keyboard.
																	// ETSITraffic is not used.
//    VehicleTrafficType[ID-1] = 0x01;
      VehicleTrafficType.push_back(0x01);							// The vector "VehicleTrafficType" is filled with 0x01 (i.e. aperiodic traffic).
    else if (PeriodicTraffic)										// Is the traffic periodic? "PeriodicTraffic" is a boolen variable acquired from keyboard.
//    VehicleTrafficType[ID-1] = 0x00;
      VehicleTrafficType.push_back(0x00);							// The vector "VehicleTrafficType" is filled with 0x00 (i.e. periodic traffic).
	/************************************************************************************************************/
	/**** From here the mixed case start to be analyzed since the traffic is neither aperiodic nor periodic. ****/
	/****          To select only a certain percentage of nodes the modulo operation is used.                ****/
	/************************************************************************************************************/
    else if (PeriodicPercentage == 10)								// Is the periodic percentage of the traffic equal to 10% (this data is acquired from keyboard)?
    {
      if (ID % 10 == 0)  											// Every 10 nodes one has periodic traffic (i.e. 0x00) the others have aperiodic traffic (i.e. 0x01).
      	VehicleTrafficType.push_back(0x00);							// The vector "VehicleTrafficType" is filled with 0x00 (i.e. periodic traffic).
      else
      	VehicleTrafficType.push_back(0x01);							// The vector "VehicleTrafficType" is filled with 0x01 (i.e. aperiodic traffic).
    }
    else if (PeriodicPercentage == 25)								// Is the periodic percentage of the traffic equal to 25% (this data is acquired from keyboard)?
    {
      if (ID % 4 == 0)       										// Every 4 nodes one has periodic traffic (i.e. 0x00) the others have aperiodic traffic (i.e. 0x01).
      	VehicleTrafficType.push_back(0x00);							// The vector "VehicleTrafficType" is filled with 0x00 (i.e. periodic traffic).
      else
      	VehicleTrafficType.push_back(0x01);							// The vector "VehicleTrafficType" is filled with 0x01 (i.e. aperiodic traffic).
    }
    else if (PeriodicPercentage == 50)								// Is the periodic percentage of the traffic equal to 50% (this data is acquired from keyboard)?
    {
//      VehicleTrafficType[ID-1] = 0x01;
      if (ID % 2 == 0)       										// One node out of two has periodic traffic (i.e. 0x00) the other has aperiodic traffic (i.e. 0x01).
         VehicleTrafficType.push_back(0x00);						// The vector "VehicleTrafficType" is filled with 0x00 (i.e. periodic traffic).
      else
         VehicleTrafficType.push_back(0x01);						// The vector "VehicleTrafficType" is filled with 0x01 (i.e. aperiodic traffic).
    }
    else if (PeriodicPercentage == 75)								// Is the periodic percentage of the traffic equal to 75% (this data is acquired from keyboard)?
    {
      if (ID % 4 == 0)       										// Every 4 nodes one has aperiodic traffic (i.e. 0x01) the others have periodic traffic (i.e. 0x00).
      	VehicleTrafficType.push_back(0x01);							// The vector "VehicleTrafficType" is filled with 0x01 (i.e. aperiodic traffic).
      else
      	VehicleTrafficType.push_back(0x00);							// The vector "VehicleTrafficType" is filled with 0x00 (i.e. periodic traffic).
    }
    else if (PeriodicPercentage == 90)								// Is the periodic percentage of the traffic equal to 90% (this data is acquired from keyboard)?
    {
      if (ID % 10 == 0)       										// Every 10 nodes one has aperiodic traffic (i.e. 0x01) the others have periodic traffic (i.e. 0x00).
      	VehicleTrafficType.push_back(0x01);							// The vector "VehicleTrafficType" is filled with 0x01 (i.e. aperiodic traffic).
      else
      	VehicleTrafficType.push_back(0x00);							// The vector "VehicleTrafficType" is filled with 0x00 (i.e. periodic traffic).
    }
//  EnableTX[ID-1] = true; //Enable a UE to transmit
    EnableTX.push_back(true); //Enable a UE to transmit				// A vector "EnableTX" is filled with a true boolean variable using the command ".push_back(true)"
  }
  /************************************************************************************************************/



//  Print(ueResponders);  				// Used to print the initial position of the nodes in the output file.



  /************************************************************************************************************/
  /*************** With this line the matrix containing channel models for 3GPP is initialized. ***************/
  /************************************************************************************************************/
  Sl3GPPChannelMatrix->InitChannelMatrix(ueResponders);
  
  
  
  ModuloSplit = 2;													// "ModuloSplit" is a int variable.
  
  
  
  /************************************************************************************************************/
  /********** The if structure is used to investigated which type of traffic is chosen by keyboard. ***********/
  /************************************************************************************************************/
   ...
   if ((MixedTraffic) || (AperiodicTraffic) || (PeriodicTraffic))
   {
     ...
     //Tgen_aperiodic_c = 50;
     RndExp = CreateObject<ExponentialRandomVariable> ();					// RndExp is a pointer of type "ExponentialRandomVariable" defined before the main.
																			// Using the function "CreateObject()" an empty object called "ExponentialRandomVariable"
																			// is initialized and the pointer to it is saved in "RndExp".
     RndExp_1 = CreateObject<ExponentialRandomVariable> ();					// RndExp_1 is a pointer of type "ExponentialRandomVariable" defined before the main.
																			// Using the function "CreateObject()" an empty object called "ExponentialRandomVariable"
																			// is initialized and the pointer to it is saved in "RndExp_1".
//   uint16_t quantizationStep = 100;
     uint16_t quantizationStep = 200;										// uint16_t variable initialization. It is used as step for the "AperiodicPKTs_Size" array.
     LargestAperiodicSize = 1200; 											// "LargestAperiodicSize" is a uint16_t variable. Maximum value of the "AperiodicPKTs_Size" array.
//   LargestAperiodicSize = 100;											// Largest packet size for aperiodic traffic.
	 /************************************************************************************************************/
	 /***********************  This for is used to fill the "AperiodicPKTs_Size" vector.   ***********************/
	 /***********************   The vector contains different sizes for traffic packets.   ***********************/
     /***********************  This vector is used in the "UdpClient" function to generate ***********************/
	 /***********************        the traffic with variable packet size.                ***********************/
	 /************************************************************************************************************/
     for(uint16_t k = 1; k <= LargestAperiodicSize/quantizationStep; k++)
     {
       if (VariablePacketSize)												// "VariablePacketSize" is a boolean variable that can be acquired from keyboard otherwise false by default.
																	
         AperiodicPKTs_Size.push_back(k*quantizationStep-35); 				// If the variable packet size is used a vector of different size elements is built.
																			// "AperiodicPKTs_Size" is a std c++ vector for uint16_t numbers, it is empty and with zero dimension.
																			// At the end of the for loop is LargestAperiodicSize/quantizationStep long.
																			// In this case v[6]=(200-35,400-35,600-35,800-35,1000-35,1200-35)
																			// 35 is subtracted to obtain packet with a size equal to 200,400,600,800,1000,1200 at application level.
																			// The function ".push_back()" is used to add new elements.
       else
         AperiodicPKTs_Size.push_back(200-35);  							// This is the case in which the variable packet size is not used,
																			// therefore the vector is filled by k elements of the same size (i.e. 200-35).
     }
	 /************************************************************************************************************/
	 /***            This for is used to fill the "PDB_Aperiodic" and "Aperiodic_Tgen_c" vectors.              ***/
	 /***            "PDB_Aperiodic" vector contains the Packet Delay Budget value for each node.              ***/
	 /***      "Aperiodic_Tgen_c" vector contains the c value for each node. Remember that: tau = c + r.       ***/
	 /*** tau --> non-deterministic packet inter-arrival time; c --> constant term; r = RndExp --> random term ***/
	 /************************************************************************************************************/
     for (NodeContainer::Iterator L = ueResponders.Begin(); L != ueResponders.End(); ++L)
     {
       int ID;														// int variable declaration.	   
       ...
       ID = node->GetId ();											// Used to select the function "GetId()" located in "node".
																	// "GetId()" returns an integer number (i.e. ID address of the node).
       if (ID % ModuloSplit != 0)  									// Every odd ID this contidion becomes true.
       {
         if (inputPDB != 0)											// "inputPDB" is a int variable initilized to 0, it can be changed by the operator using keyboard. If the
																	// Packet Delay Budget is different from 0, so if it is set by keyboard, then the if condition is satisfied.
           PDB_Aperiodic.push_back(inputPDB);						// "PDB_Aperiodic" is a std c++ vector made of double numbers.
																	// The inputPDB acquired from keyboard is added to the "PDB_Aperiodic" vector.
         else
           PDB_Aperiodic.push_back(50);								// This is the case in which the "inputPDB" is equal to 0 by default,
																	// therefore a 50 is push_back to the "PDB_Aperiodic" vector.
         Aperiodic_Tgen_c.push_back(50);							// The "Aperiodic_Tgen_c" is filled with a 50. This is a vector that contains c (where c is the )
         RndExp->SetAttribute ("Mean", DoubleValue(50));			// "SetAttribute()" is a function that sets the mean value of the RndExp.
																	// "DoubleValue()" is a function used to set a double value (50 in this case).
//         Aperiodic_Tgen_c.push_back(5);
//         RndExp->SetAttribute ("Mean", DoubleValue(5));
       }
       else															// This is the case of even ID.
       {
         if (inputPDB != 0)											// If the PDB is different from 0, so if it is set by keyboard, then the if condition is satisfied.
           PDB_Aperiodic.push_back(inputPDB);						// "PDB_Aperiodic" is a std c++ vector made of double numbers.
																	// The inputPDB acquired from keyboard is added to the "PDB_Aperiodic" vector.
         else
           PDB_Aperiodic.push_back(50);								// This is the case in which the "inputPDB" is equal to 0 by default,
																	// therefore a 50 is push_back to the "PDB_Aperiodic" vector.
         Aperiodic_Tgen_c.push_back(50);							// The "Aperiodic_Tgen_c" is filled with a 50.
         RndExp_1->SetAttribute ("Mean", DoubleValue(50));			// "SetAttribute()" is a function of the class "ObjectBase" so it is inherit by the object "ExponentialRandomVariable"
																	// It defines a new attribute in the memory cell pointed by RndExp_1.
																	// The new attribute is called "Mean". "DoubleValue()" is a function used to set a double value (50 in this case).
...
//         RndExp_1->SetAttribute ("Mean", DoubleValue(25));
	 ...
	 /************************************************************************************************************/
	 /***             This for is used to fill the "PDB_Periodic" and "Periodic_Tgen_c" vectors.               ***/
	 /***             "PDB_Periodic" vector contains the Packet Delay Budget value for each node.              ***/
	 /***       "Periodic_Tgen_c" vector contains the c value for each node. Remember that: tau = c + r.       ***/
	 /*** tau --> non-deterministic packet inter-arrival time; c --> constant term; r = RndExp --> random term ***/
	 /************************************************************************************************************/
     for (NodeContainer::Iterator L = ueResponders.Begin(); L != ueResponders.End(); ++L)
     {
       ...
       ID = node->GetId ();											// Used to select the function "GetId()" located in "node".
																	// "GetId()" returns an integer number (i.e. ID address of the node).
       if (ID % 2 == 0)   											// Every odd ID this contidion becomes true.
       {
         if (inputPDB != 0)											// "inputPDB" is a int variable initilized to 0, it can be changed by the operator using keyboard. If the
																	// Packet Delay Budget is different from 0, so if it is set by keyboard, then the if condition is satisfied.
           PDB_Periodic.push_back(inputPDB);						// "PDB_Periodic" is a std c++ vector made of double numbers.
																	// The inputPDB acquired from keyboard is added to the "PDB_Periodic" vector.
         else
           PDB_Periodic.push_back(100);								// This is the case in which the "inputPDB" is equal to 0 by default,
																	// therefore a 100 is push_back to the "PDB_Periodic" vector.
         Periodic_Tgen.push_back(100);								// The "Periodic_Tgen_c" is filled with a 100.
       }
       else															// This is the case of even ID.
       {
         if (inputPDB != 0)											// If the PDB is different from 0, so if it is set by keyboard, then the if condition is satisfied.
...
//   PeriodicPKTs_Size = {300-34, 300-34, 300-34, 300-34 ,300-34}; 	// Account for the overhead
     PeriodicPKTs_Size = {200-35, 200-35, 200-35, 200-35 ,200-35}; 		// Account for the overhead per layer header
																		// "PeriodicPKTs_Size" is a std c++ vector for uint16_t numbers.
     LargestPeriodicSize = PeriodicPKTs_Size[0];  						// 300 bytes is the largest packet size for aperiodic traffic, according to the 3GPP standard.
     Ptr<UniformRandomVariable> random_index = CreateObject<UniformRandomVariable>();	// A new object of type "UniformRandomVariable" is created and "random_index" is its pointer.
	 /************************************************************************************************************/
	 /************ This for is used create a new vector called "Pattern_index". A random index of the ************/
	 /************ "PeriodicPKTs_Size" vector is saved for each node. In this way each node can have  ************/
	 /************          different value of packet size. In order to genearate a periodic          ************/
	 /************             traffic nodes take a random index from "PeriodicPKTs_Size"             ************/
	 /************************************************************************************************************/
     for (NodeContainer::Iterator NN = ueResponders.Begin(); NN != ueResponders.End(); ++NN)  	// UEs are indexed starting from 1
     {  
       Pattern_index.push_back(random_index->GetInteger(0,PeriodicPKTs_Size.size()-1) );		// "Pattern_index" is an empty vector for uint16_t numbers.
																								// A random integer value is chosen in between 0 and the
																								// size and of "PeriodicPKTs_Size" decremented by 1
     }
   }
   else 	/*** Not Mixed traffic, not Aperiodic traffic, not Periodic traffic; It is ETSI traffic, not used in our case. ***/
   {
     for (NodeContainer::Iterator NN = ueResponders.Begin(); NN != ueResponders.End(); ++NN)
     {  
       Pattern_index.push_back(0);				// "Pattern_index" is an empty vector for uint16_t numbers. It is filled with 0.
     }
     LargestCAMSize = 850;						// "LargestCAMSize" is a uint16_t variable. It memorizes the the value of the largest CAM.
...
//   std::cin.get();
   ...
   MeasInterval = ((double)TrepPrint)/1000;									// double is used to change the type of TrepPrint which is initialized as int.
																			// MeasInterval = 100 / 1000 = 0,1 ms.
  //mobility.SetPositionAllocator (positionAlloc);
  NS_LOG_INFO ("Installing UE network devices...");							// Used to print a message.
  NetDeviceContainer ueDevs = lteHelper->InstallUeDevice(ueResponders);		// ueDevs is a container for the network devices installed on the different nodes (UeDevs).
																			// Each node can have more network devices (e.i. network devices = interfaces).
																			// The function "InstallUeDevice()" belongs to the object pointed by "LteHelper".
																			// The argument "ueResponders" is used beacuse the lte devices have to be 
																			// installed on the node belonging to the node container.



  /************************************************************************************************************/
  /**************** This for loop is used to retrieve for each the network device of interest. ****************/
  /****************                     In our case, LTE device --> 0                          ****************/
  /****************        The for loop is also used to retrieve the NIST net device           ****************/
  /****************        NIST is a group that modified some portions of the stack.           ****************/
  /************************************************************************************************************/
  for (NodeContainer::Iterator L = ueResponders.Begin(); L != ueResponders.End(); ++L)
  {
    Ptr<Node> node = *L; 														// Used to scan all nodes in NodeContainer.
    NS_LOG_INFO("Node " << node->GetId() << ", " << node->GetNDevices());		// It prints the node ID and the number of devices connected to that node.
    Ptr<NetDevice> dev = node->GetDevice(0);									// "dev" is a pointer of "NetDevice" type. In "dev" is saved the ponter to the LTE device.
    Ptr<NistLteUeNetDevice> netDev = dev->GetObject<NistLteUeNetDevice>();		// "netDev" is a pointer of "NistLteUeNetDevice". Using the pointer dev it is possible to
																				// retrieve the pointer to the object "NistLteUeNetDevice" for each node.
    Ptr<NrV2XUeMac> mac = netDev->GetMac();										// "mac" is a pointer of "NrV2XUeMac" type. Using this pointer it is possible to
																				//	retrieve the pointer to the MAC for each node.
    if (FrequencyReuse)
      mac->CopySubchannelsMap(SubchannelIndexMap);
    if (PeriodicTraffic)								/*** This is the case of Periodic traffic. ***/
														// The if structure is used to fill the "m_RRIvalues" vector.
      mac->PushNewRRIValue(100);						// The "m_RRIvalues" vector is defined in the "nr-v2x-ue-mac.cc". The maximum vector size is 16.
														// Using the function "PushNewRRIValue()" it is possible to push back the desired value of RRI.
														// In the case of Periodic traffic the "m_RRIvalues" vector has size equal to 1, for each node.
    else if (AperiodicTraffic)							/*** This is the case of Aperiodic traffic. ***/
														// Each node has its own "m_RRIvalues" vector.
														// The vector is defined in the "nr-v2x-ue-mac.cc". The maximum vector size is 16.
														// In the case of Aperiodic traffic the vector has size equal to 2, for each node.
    {
      if (avgRRI)										// "avgRRI" is a boolen variable defined before the main. It is false by default. It can also be acquired by keyboard.
														// "avgRRI" stands for average RRI in case of aperiodic traffic. It is used to reserve resources.
														
      {
        mac->PushNewRRIValue(RndExp->GetMean()*2);		// With this command the mean value of the "RndExp" multiplied by 2 is pushed back in the "m_RRIvalues" vector.
														// Remember that "RndExp" is a pointer therfore is needed the arrow operator to get its content. 100 ms.
        mac->PushNewRRIValue(RndExp_1->GetMean()*2);	// With this command the mean value of the "RndExp_1" multiplied by 2 is pushed back in the "m_RRIvalues" vector.
														// Remember that "RndExp_1" is a pointer therfore is needed the arrow operator to get its content. 100 ms.
      }
      else												// If the "avgRRI" is not acquired by keyboard.
      {
        mac->PushNewRRIValue(RndExp->GetMean());		// With this command the mean value of the "RndExp" is pushed back in the "m_RRIvalues" vector.
														// Remember that "RndExp" is a pointer therfore is needed the arrow operator to get its content. 50 ms.
        mac->PushNewRRIValue(RndExp_1->GetMean());		// With this command the mean value of the "RndExp_1" is pushed back in the "m_RRIvalues" vector.
														// Remember that "RndExp_1" is a pointer therfore is needed the arrow operator to get its content. 50 ms.
      }
    }
    else												/*** This is the case of Mixed traffic. ***/
														// In the case of Mixed traffic the "m_RRIvalues" vector has size equal to 3, for each node.
    {
      mac->PushNewRRIValue(100);						// The value 100 (ms) is pushed back in the "m_RRIvalues" vector.
      if (avgRRI)										// If the "avgRRI" is enabled this condition becomes true.
      {
        mac->PushNewRRIValue(RndExp->GetMean()*2);		// With this command the mean value of the "RndExp" multiplied by 2 is pushed back in the "m_RRIvalues" vector.
														// Remember that "RndExp" is a pointer therfore is needed the arrow operator to get its content. 100 ms.
        mac->PushNewRRIValue(RndExp_1->GetMean()*2);	// With this command the mean value of the "RndExp_1" multiplied by 2 is pushed back in the "m_RRIvalues" vector.
...
//      mac->PushNewRRIValue(10);
  ...
  /*** The following lines are used to install the IP stack ***/
  NS_LOG_INFO ("Installing IP stack...");
  InternetStackHelper internet;				// Even in this case a Helper is needed.
  internet.Install (ueResponders);
  
  
  
  /*** Now, the IP addresses are allocated and a network route is created as well. ***/
  ...
  Ipv4StaticRoutingHelper ipv4RoutingHelper;				 	// This is a Helper.
  Ipv4AddressHelper ipv4;										// This is a Helper.
  NS_LOG_INFO ("Assign IP Addresses.");
  ipv4.SetBase ("10.1.0.0", "255.255.0.0");						// The "Setbase()" function is used to set: base network number, network mask, and base address.
...
//Ipv4InterfaceContainer intcont2 = ipv4.Assign (ueSendersDevs); 
  ...
  /************************************************************************************************************/
  /***********            This for loop is used to set up the IP network seen from the eNB.         ***********/
  /************************************************************************************************************/
...
//    std::cout << ueNode -> GetObject<Ipv4> () -> GetAddress(1,0).GetLocal() << std::endl;
  ...
  /************************************************************************************************************/
  /*********** A Helper (i.e. lteHelper) is used to connect the UEs to the lte network. The role of ***********/
  /***********  such helper is to create various LTE objects and arrange them together to make the  ***********/
  /***********   whole LTE system (downlink spectrum channel, uplink spectrum channel, eNBs etc.).  ***********/
  /************************************************************************************************************/
  ...
  /************************************************************************************************************/
  /********************* With the following lines applications are installed on the UEs. **********************/
  /*********************        An helper called "NrV2XUdpClientHelper" is used.         **********************/
  /************************************************************************************************************/
...
//groupAddress = Ipv4AddressGenerator::NextAddress (Ipv4Mask ("255.0.0.0"));
  ...
  UdpClientHelper udpClient (groupAddress , 8000); 								// Set destination IP address and UDP port (8000 in this case).
																				// The group address is used to set the Sidelink Bearers.
...
/************************************************************************************************************/
/**************    In the following lines are set attributes for the NrV2XUdpClient class.    **************/
/************** It is possible to distinguish between periodic traffic and aperiodic traffic. ***************/
/************************************************************************************************************/
...
//@OLD  clientApps.Get(0) -> SetStopTime (Seconds (simTime + 1));
  ...
  /************************************************************************************************************/
  /************** Receiver side, a packet sink application is installed. This is an application ***************/
  /**************      that only receives packets from the transmitting UEs (vehicles).         ***************/
  /************************************************************************************************************/
 ...
 /************************************************************************************************************/
 /************************ With the following code, the PSSCH bitmap value is built.  ************************/
 /************************ The decision here concern the use the overall grid or not. ************************/
 /************************************************************************************************************/
...
//  std::cin.get();
 ...
 /************************************************************************************************************/
 /********************** This part of the code is in charge of starting the simulation. **********************/
 /**********************        The simulation is stopped after simTime+1 seconds.      **********************/
 /************************************************************************************************************/
  NS_LOG_INFO ("Starting simulation...");
  Simulator::Stop (Seconds (simTime+1));    //stoppa la simulazione
  Simulator::Run ();
  /*
