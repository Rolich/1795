...
RC = 515
distance = 100
filename = "PLR_matrix_100m_RC" + str(RC) + "_weight.txt"

# Assuming your text file has no header and is comma-separated
# Provide column names explicitly
column_names = ['SimulationNumber', 'Distance', 'PLR', 'Pkeep', 'Weight']
df = pd.read_csv(filename, delimiter=',', names=column_names)

# Filter data where 'PLR' is greater than 0 and 'Distance' is less than or equal to the specified distance
filtered_df = df[(df['PLR'] >= 0) & (df['Distance'] <= distance)]

# Group by 'Distance' and 'Pkeep', calculate mean of 'PAoI' and 'Weight' for each group
result_df = filtered_df.groupby(['Distance', 'Pkeep'], as_index=False).agg({'PLR': 'mean', 'Weight': 'mean'})

result_df.sort_values(by=['Pkeep', 'Distance'], inplace=True)

# Write the result to a new text file
output_file_path = "PLR_Weight_matrix_mean_RC" + str(RC) + ".txt"
result_df.to_csv(output_file_path, sep=',', index=False)

data = np.genfromtxt(output_file_path, delimiter=',', names=['Distance', 'Pkeep', 'PLR', 'Weight'])

# Filter out rows where Pkeep is zero
filtered_data = data[(data['PLR'] >= 0) & (data['Distance'] <= distance)]
...
for pkeep in pkeep_values:
    ...
    if subset_size > 0:
        weighted_average_paio += np.sum(subset_data['PLR'] * subset_data['Weight'])
...
plt.savefig('meanPLR_vs_Pkeep_100m_Distance' + str(distance) +'RC'+ str(RC)  +'.png')
plt.savefig('meanPLR_vs_Pkeep_100m_Distance' + str(distance) +'RC'+ str(RC)  +'.eps')
# Show the plot
plt.show()
...
for pkeep in pkeep_values:
    ...
    if pkeep == 0.99:
        plt.plot(subset_data['Distance'], subset_data['PLR'], label=f'Pkeep = {pkeep}', marker='o')
    else:
        plt.plot(subset_data['Distance'], subset_data['PLR'], label=f'Pkeep = {pkeep}')
...
plt.ylabel('PLR')
plt.legend()
plt.title('PLR as a function of Distance. Distance=<' + str(distance))

plt.savefig('PLR_vs_Distance_100m_10pkeep_Distance' + str(distance) +'RC'+ str(RC)  +'.png')
plt.savefig('PLR_vs_Distance_100m_10pkeep_Distance' + str(distance) +'RC'+ str(RC)  +'.eps')
# Show the plot
plt.show()
...
plt.savefig('PLR_vs_Distance_100m_Distance' + str(distance) +'RC'+ str(RC)  +'.png')
plt.savefig('PLR_vs_Distance_100m_Distance' + str(distance) +'RC'+ str(RC)  +'.eps')
