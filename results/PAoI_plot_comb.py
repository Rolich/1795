...
filename1 = "PAoI_Weight_matrix_mean_RC515.txt"
data1 = np.genfromtxt(filename1, delimiter=',', names=['Distance', 'Pkeep', 'PAoI', 'Weight'])
distance = 920

# Read data from the second file
filename2 = "PAoI_Weight_matrix_mean_RC1.txt"  # Replace with the actual second file name
data2 = np.genfromtxt(filename2, delimiter=',', names=['Distance', 'Pkeep', 'PAoI', 'Weight'])

# Filter out rows where Pkeep is zero for both datasets
filtered_data1 = data1[(data1['PAoI'] > 0) & (data1['Distance'] <= distance)]
filtered_data2 = data2[(data2['PAoI'] > 0) & (data2['Distance'] <= distance)]
...
for pkeep in pkeep_values1:
    ...
    average_paio_per_distance1.append((pkeep, average_paio*1000))
...
for pkeep in pkeep_values2:
    ...
    average_paio = np.mean(subset_data['PAoI'])
    average_paio_per_distance2.append((pkeep, average_paio*1000))
...
plt.savefig('meanPAoI_vs_Pkeep_RC1vsRC515_Distance' + str(distance)+'.eps')
plt.savefig('meanPAoI_vs_Pkeep_RC1vsRC515_Distance' + str(distance)+'.png')
# Show the plot
plt.show()